...
public:   
   ...
   int checkChutesLadders(int position) {
      ...
      // TODO: implement this function properly
      //throw std::logic_error("not implemented yet");
	  if (laddersSquares[position] > 0)
		  return laddersSquares[position];
	  if (chutesSquares[position] > 0)
		  return chutesSquares[position];
	  return position;
   }
...
private:
   ...
   // Requirement: use ExtendableVector to store the square
	ExtendableVector<int> gameSquares = ExtendableVector<int>(101);
	ExtendableVector<int> laddersSquares = ExtendableVector<int>(101);
	ExtendableVector<int> chutesSquares = ExtendableVector<int>(101);
};